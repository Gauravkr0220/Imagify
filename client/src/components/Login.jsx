...
const Login = () => {
    ...
    const onSubmitHandler=async(e)=>{
        e.preventDefault(); // prevent webpage for reloading
        try{
            if(state=='Login'){
    ...
    useEffect(() => {
        document.body.style.overflow = 'hidden'; //to remove the scrolling of page when it is open
        return () => {
    ...
    return (
        <div className='fixed top-0 left-0 right-0 bottom-0 z-10 backdrop-blur-sm bg-black/30 flex justify-center items-center'>
            ...
            className='relative bg-white p-10 rounded-xl text-slate-500'>
                ...
                <button className='bg-blue-600 w-full text-white py-2 my-2 rounded-full'>{state === 'Login' ? 'Login' : 'Create Account'}</button>
